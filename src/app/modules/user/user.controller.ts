...
const getOrdersFromUser = async (req: Request, res: Response) => {
  try {
    const userId = req.params.userId;
    const userIsExists = await User.isUserExists(Number(userId));
    if (!userIsExists) {
    ...
    const orders = await userServices.getOrdersFromUserFromDB(Number(userId));
...
const getTotalPriceByUserOrders = async (req: Request, res: Response) => {
  try {
    const userId = req.params.userId;
    const userIsExists = await User.isUserExists(Number(userId));
    if (!userIsExists) {
    ...
    const result = await userServices.getTotalPriceByUserOrdersFromDB(
      Number(userId),
    );
